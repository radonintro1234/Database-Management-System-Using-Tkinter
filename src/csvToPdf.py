...
def makecsv():
    ...
    try:
        conn = sqlite3.connect(
            'inventory.db', isolation_level=None, detect_types=sqlite3.PARSE_COLNAMES)
        db_df = pd.read_sql_query("SELECT * FROM books_data", conn)
...
def CsvtoPDF():
    ...
    try:
        ...
        filename = filedialog.asksaveasfilename(defaultextension=".pdf", filetypes=(
        ...
        with open('database.csv', newline='') as f:
            ...
            pdf.cell(page_width, 0.0, 'Book Data', align='C')
            pdf.ln(10)

            pdf.set_font('Courier', '', 12)

            col_width = page_width/int(ncol - 1)
            ...
            for row in reader:
                ...
                for x in range(int(ncol - 1)):
                    if x == 0 or x == 3 or x == 6:
                        pdf.cell(col_width * 0.5, th,
                                 str(row[x]), border=1, align='C')
                    elif x == 1:
                        pdf.cell(col_width * 2, th,
                                 str(row[x]), border=1, align='C')
                    else:
                        pdf.cell(col_width, th, str(
                            row[x]), border=1, align='C')
            ...
            pdf.cell(page_width, 0.0, '- Happy Reading! -', align='C')

            pdf.output(filename, 'F')

        messagebox.showinfo(
            "Success", f"Data Successfully Exported to '{filename}' !!!")
